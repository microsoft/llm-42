...
# Mode definitions
mode_labels = {
    'baseline_nondet': 'SGLang (Non-deterministic)',
    'det_mode_257': 'Sglang (Deterministic)',
    'det_mode_66': 'BI: Ours',
...
sorted_configs = ['baseline_nondet@100%', 'det_mode_257@100%', 'det_mode_578@0%', 'det_mode_578@5%', 'det_mode_578@10%']

pct_names = ['pct_0', 'pct_5', 'pct_10', 'pct_100']
...
def get_config_label(config: str) -> str:
    ...
    if 'baseline_nondet' in config:
        ...
        return f'DetInfer ({pct}% deterministic)'
    else:
...
def plot_cdf_comparison(raw_data: Dict[str, Dict], output_dir: Path):
    ...
    for metric_key, metric_name, xlabel in metrics:
        ...
        # Save plot as PDF
        output_file_pdf = output_dir / f'{metric_key}_cdf_comparison.png'
...
def plot_bar_comparison(results: Dict[str, Dict], output_dir: Path):
    ...
    output_file_pdf = output_dir / 'performance_summary.png'
    plt.savefig(output_file_pdf, bbox_inches='tight', dpi=150)
...
def plot_overhead_comparison(results: Dict[str, Dict], output_dir: Path):
    ...
    output_file_pdf = output_dir / 'component_overhead_comparison.png'
    plt.savefig(output_file_pdf, bbox_inches='tight', dpi=150)
