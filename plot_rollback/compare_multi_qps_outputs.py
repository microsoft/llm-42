...
def run_experiment_process(idx: int, base: dict, url: str, qps: float, output_dir: Path, 
    ...
    print(f"[{idx+1}] Starting QPS={qps} on {url}...")
...
def main():
    ...
    print("Launching all experiments in parallel processes...")
    print()
    ...
    tokenized_outputs = [(r["qps"], r["tokens"], r["texts"]) for r in process_results]
    ...
    for i in range(num_runs):
        for j in range(i + 1, num_runs):
            qps_i, tokens_i, texts_i = tokenized_outputs[i]
            qps_j, tokens_j, texts_j = tokenized_outputs[j]
            
            mismatches = []
            for req_id, (tok_i, tok_j, text_i, text_j) in enumerate(zip(tokens_i, tokens_j, texts_i, texts_j)):
            ...
            # Calculate mismatch fraction
            deltas = [m["delta"] for m in mismatches]
            mismatch_frac = 1.0 - np.mean(np.array(deltas) == 0)
