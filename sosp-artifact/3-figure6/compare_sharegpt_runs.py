...
compare token-level outputs and record the first and second consistent spans.

Writes per-request results to mismatch_per_request.jsonl and a summary.json. Use
plot.py to render the CDF (the paper's Figure 6) from the jsonl.
"""
...
def main():
    ...
    # Use actual output token IDs from bench_serving instead of re-tokenizing
    seq_tokens = seq_result["output_ids"]
    poi_tokens = poi_result["output_ids"]
    ...
    for i, (s_tok, p_tok, s_text, p_text) in enumerate(
        ...
        delta = output_length - first_mismatch
        deltas.append(delta)
        # Consistent spans: tokens that agree up to the first / between the first and
        # second mismatch (a fully-consistent request has first_span == output_length).
        first_span = output_length if first_mismatch == output_length else first_mismatch + 1
        second_span = second_mismatch - first_mismatch
        mismatches.append(
            {
                "request_id": i,
                ...
                "first_consistent_span": first_span,
                "second_consistent_span": second_span,
            }
        )
