...
class DummyTreeCache(SimpleNamespace):
    """Minimal tree cache stub for benchmarking (no actual caching)."""
...
def run_forward_pass(reqs: List[Req], model_runner: ModelRunner):
    ...
    dummy_tree_cache = DummyTreeCache(
...
def worker(server_args, port_args, bench_args, gpu_id, tp_rank):
    """Worker function that runs on each TP rank."""
    is_rank0 = tp_rank == 0
    rank_print = print if is_rank0 else lambda *a, **kw: None

    rank_print(f"Loading model (tp_size={server_args.tp_size})...")
    ...
    rank_print(f"Running benchmarks (batch_size={batch_size}):")
    for input_len in bench_args["input_lens"]:
        result = benchmark_input_len(
            model_runner=model_runner,
            input_len=input_len,
            batch_size=batch_size,
            warmup_iters=bench_args["warmup_iters"],
            bench_iters=bench_args["bench_iters"],
            device=server_args.device,
        )
        results.append(result)

    if is_rank0:
        result_file = bench_args["result_file"]
...
def main():
    ...
    if server_args.tp_size == 1:
        worker(server_args, port_args, bench_args, gpu_id=0, tp_rank=0)
    else:
