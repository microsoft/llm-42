...
# Raise open file limit for high-concurrency benchmarks (request_rate=inf)
ulimit -n 65536 2>/dev/null || true

# GPU-balanced offline throughput benchmarks
#
# Instead of pinning one server config per GPU (which leaves baseline GPUs idle
# while LLM42 GPUs work through many det_ratios), this script:
#   1. Generates all (config, det_ratio) job pairs
#   2. Sorts them by config name to maximize server reuse
#   3. Distributes them across GPUs via a shared job queue
#   4. Each GPU worker: claim job → start/reuse server → benchmark → repeat
#
# Usage:
#   ./run_balanced_benchmark.sh                    # uses defaults
#   ./run_balanced_benchmark.sh --force           # force re-run completed experiments
#   NUM_GPUS=8 CONFIG_NAMES=... ./run_balanced_benchmark.sh
...
# Derive short model tag for log directory naming (e.g. meta-llama/Llama-3.1-8B-Instruct → llama-3.1-8b-instruct)
...
SERVER_LOG_DIR="${SERVER_LOG_DIR:-}"  # resolved after RESULTS_ROOT is set

if [ "$_GPU_TYPE" = "b200" ]; then
    CONFIG_NAMES="${CONFIG_NAMES:-sglang_non_deterministic,sglang_global_deterministic,sglang_global_deterministic_triton,llm42_ws_32_bs_32,llm42_ws_64_bs_16}"
else
    CONFIG_NAMES="${CONFIG_NAMES:-sglang_non_deterministic,sglang_global_deterministic,sglang_global_deterministic_triton,llm42_ws_32_bs_16}"
fi
...
# DATASET_CONFIGS_LIST (space-separated dataset tags) enables cross-workload
# parallelization: jobs for ALL datasets share the same GPU job queue.
# Falls back to single-dataset mode from DATASET_NAME/RANDOM_INPUT_LEN/RANDOM_OUTPUT_LEN.
...
# Helper: output dir for a given dataset tag
get_dataset_output_dir() {
    echo "${RESULTS_ROOT}/${1}_n${NUM_PROMPTS}"
}

# Output directory
if [ -z "${RESULTS_ROOT:-}" ]; then
    RESULTS_ROOT="${ROOT}/runs/${_GPU_TYPE}_${_MODEL_TAG}-tp${TP_SIZE}_${_TIMESTAMP}/results"
fi

# Server logs live inside the run directory (sibling to results/)
if [ -z "$SERVER_LOG_DIR" ]; then
    SERVER_LOG_DIR="$(dirname "$RESULTS_ROOT")/server_logs"
fi
...
for _dtag in "${DATASET_TAGS[@]}"; do
    mkdir -p "$(get_dataset_output_dir "$_dtag")"
done
mkdir -p "$SERVER_LOG_DIR"

# ---- Detect already-completed jobs from previous runs ----
# Check both summary.csv (from previous completed runs) and benchmark_results.jsonl
# for each dataset output directory.
declare -A COMPLETED_JOBS
if [ "$FORCE" -eq 1 ]; then
    echo "  --force: skipping completion detection, all jobs will run"
else
for _dtag in "${DATASET_TAGS[@]}"; do
    _ds_output_dir="$(get_dataset_output_dir "$_dtag")"
    _ds_summary="${_ds_output_dir}/summary.csv"
    _ds_results="${_ds_output_dir}/benchmark_results.jsonl"

    if [ -f "$_ds_summary" ]; then
        # summary.csv format: config,tokens-per-second,...
        # config is e.g. "sglang_non_deterministic" or "llm42_ws_32_bs_16_ratio_0.02"
        while IFS=, read -r _config _rest; do
            _config="${_config//$'\r'/}"
            [ "$_config" = "config" ] && continue  # skip header
            if [[ "$_config" == *"_ratio_"* ]]; then
                _cname="${_config%_ratio_*}"
                _dratio="${_config##*_ratio_}"
            else
                _cname="$_config"
                _dratio="1.0"
            fi
            COMPLETED_JOBS["${_cname}|${_dratio}|${_dtag}"]=1
        done < "$_ds_summary"
    fi
    if [ -f "$_ds_results" ]; then
        while IFS= read -r _line; do
            _cname=$(echo "$_line" | $PYTHON_CMD -c "import sys,json; d=json.loads(sys.stdin.read()); print(d['config_name'])" 2>/dev/null || true)
            _dratio=$(echo "$_line" | $PYTHON_CMD -c "import sys,json; d=json.loads(sys.stdin.read()); print(d['deterministic_ratio'])" 2>/dev/null || true)
            if [ -n "$_cname" ] && [ -n "$_dratio" ]; then
                COMPLETED_JOBS["${_cname}|${_dratio}|${_dtag}"]=1
            fi
        done < "$_ds_results"
    fi
...
# Generate all (config, det_ratio, dataset) jobs, skipping already-completed ones
declare -a ALL_JOBS=()
SKIPPED_JOBS=0
    ...
    for config_name in "${CONFIG_ARRAY[@]}"; do
        if [[ "$config_name" == *"llm42"* ]]; then
            ...
            for ratio in "${_RATIO_ARRAY[@]}"; do
                if [ -n "${COMPLETED_JOBS["${config_name}|${ratio}|${dataset_tag}"]:-}" ]; then
                    echo "  Skipping (already done): $config_name det_ratio=$ratio dataset=$dataset_tag"
                    SKIPPED_JOBS=$((SKIPPED_JOBS + 1))
                else
                    ALL_JOBS+=("${config_name} ${ratio} ${dataset_tag}")
                fi
            done
        else
            if [ -n "${COMPLETED_JOBS["${config_name}|${BASELINE_RATIOS}|${dataset_tag}"]:-}" ]; then
                echo "  Skipping (already done): $config_name det_ratio=$BASELINE_RATIOS dataset=$dataset_tag"
                SKIPPED_JOBS=$((SKIPPED_JOBS + 1))
            else
                ALL_JOBS+=("${config_name} ${BASELINE_RATIOS} ${dataset_tag}")
            fi
        fi
    done
done
...
# Dataset changes don't require server restarts, only config changes do.
...
# ---- Display ----
echo "=============================================="
echo "GPU-Balanced Offline Throughput Benchmark"
echo "=============================================="
echo "Model: $MODEL"
...
cleanup() {
    echo ""
    echo "Cleaning up..."
    # Kill workers first so they stop claiming new jobs
...
get_config_args() {
    ...
    case "$config_name" in
        ...
        "sglang_global_deterministic_triton")
            ...
            ;;
        llm42_ws_*_bs_*)
            # Parse window size and batch size from config name: llm42_ws_<W>_bs_<B>
            local ws bs
...
# Prints "config_name det_ratio" on success; returns 1 when queue is empty.
claim_next_job() {
    while true; do
        local next_job
...
run_single_benchmark() {
    ...
    local ds_output_dir
    ds_output_dir="$(get_dataset_output_dir "$dataset_tag")"

    local slot_results="${ds_output_dir}/results_slot${slot_id}.jsonl"
    local temp_result="${ds_output_dir}/temp_${config_name}_det${det_ratio}.jsonl"

    echo "[Slot $slot_id] Running: $config_name det_ratio=$det_ratio dataset=$dataset_tag"
    ...
    # Build dataset args
    # Check for shared prepared datasets (created by prepare_shared_datasets.py)
    ...
    if [ "$ds_name" = "random" ]; then
        ...
        # Use shared prepared dataset (openai chat format, same prompts as vLLM)
        # Must use openai-chat backend so messages go through /v1/chat/completions
    ...
    $PYTHON_CMD -m sglang.bench_serving \
        ...
        --output-details \
        2>&1 | tee "${ds_output_dir}/log_${config_name}_det${det_ratio}.log" || bench_rc=$?

    if [ "$bench_rc" -ne 0 ]; then
        echo "[Slot $slot_id] WARNING: benchmark exited with code $bench_rc for $config_name det_ratio=$det_ratio"
    fi
    ...
    for line in f:
        ...
        if line:
            try:
                ...
                # Extract per-request rollback arrays before stripping meta_info
                ...
                pass
" >> "$slot_results" || echo "[Slot $slot_id] WARNING: failed to process results for $config_name det_ratio=$det_ratio"
        rm -f "$temp_result"
    fi

    echo "[Slot $slot_id] Completed: $config_name det_ratio=$det_ratio dataset=$dataset_tag"
}

# ---- GPU Worker ----
# Builds a comma-separated CUDA_VISIBLE_DEVICES string for a given slot.
# Slot 0 with TP_SIZE=4 → "0,1,2,3"; Slot 1 → "4,5,6,7"; etc.
get_slot_devices() {
    local slot_id=$1
...
# Each worker claims jobs from the shared queue, launches/reuses servers, and
# runs benchmarks until the queue is exhausted.
gpu_worker() {
    local slot_id=$1
    local devices
    ...
    # Kill our server on exit (handles Ctrl-C propagation)
    ...
    while true; do
        ...
        local config_name det_ratio dataset_tag
        config_name=$(echo "$job_info" | awk '{print $1}')
        det_ratio=$(echo "$job_info" | awk '{print $2}')
        dataset_tag=$(echo "$job_info" | awk '{print $3}')

        echo "[$label] Claimed: $config_name det_ratio=$det_ratio dataset=$dataset_tag"

        # Ensure a healthy server is running for this config.
        # If the config changed we must restart; if it didn't change we still
        # verify the existing server is alive (it may have crashed between jobs).
        ...
        if [ "$need_restart" -eq 1 ]; then
            ...
            local log_file="$SERVER_LOG_DIR/server_slot${slot_id}_${config_name}.log"
            ...
            if ! wait_for_server "$url" "$SERVER_STARTUP_TIMEOUT"; then
                ...
                echo "[$label] Skipping job: $config_name det_ratio=$det_ratio"
                kill_server "$server_pid"
                server_pid=""
                current_config=""
                continue
        ...
        run_single_benchmark "$url" "$config_name" "$det_ratio" "$slot_id" "$dataset_tag" || 
            echo "[$label] WARNING: benchmark failed for $config_name det_ratio=$det_ratio dataset=$dataset_tag"
    done
...
# Pre-flight: kill any stale processes on our ports from previous runs
...
# Merge per-GPU result files into the final results file per dataset (append to preserve previous results)
echo ""
echo "Merging results..."
for _dtag in "${DATASET_TAGS[@]}"; do
    _ds_output_dir="$(get_dataset_output_dir "$_dtag")"
    _ds_results_file="${_ds_output_dir}/benchmark_results.jsonl"
    for ((i=0; i<NUM_WORKERS; i++)); do
        slot_results="${_ds_output_dir}/results_slot${i}.jsonl"
...
echo "Results root: $RESULTS_ROOT"
for _dtag in "${DATASET_TAGS[@]}"; do
    echo "  $_dtag: $(get_dataset_output_dir "$_dtag")/benchmark_results.jsonl"
done
echo "=============================================="
...
for _dtag in "${DATASET_TAGS[@]}"; do
    ...
    if [ -f "$_ds_results_file" ]; then
        CSV_FILE="${_ds_output_dir}/per_request_data.csv"
        $PYTHON_CMD "${ROOT}/export_per_request_csv.py" \
            --input "$_ds_results_file" \
            --output "$CSV_FILE"

        SUMMARY_CSV="${_ds_output_dir}/summary.csv"
