...
# This script sends requests to already running servers (launched via launch_servers_parallel.sh)
# and collects throughput metrics.
...
CONFIG_NAMES=${CONFIG_NAMES:-"sglang_non_deterministic,sglang_global_deterministic,detinfer_ws_32_bs_16,detinfer_ws_16_bs_32"}

# Benchmark parameters
MODEL=${MODEL:-meta-llama/Llama-3.1-8B-Instruct}
TOKENIZER=${TOKENIZER:-}
NUM_PROMPTS=${NUM_PROMPTS:-16384}
DATASET_NAME=${DATASET_NAME:-sharegpt}  # sharegpt or random
...
DETERMINISTIC_RATIOS="${DETERMINISTIC_RATIOS:-0.0 0.1 0.2 0.5 1.0}"
...
echo "=============================================="
echo "Offline Throughput Benchmark"
echo "=============================================="
echo "Model: $MODEL"
...
echo "Deterministic Ratios: $DETERMINISTIC_RATIOS"
...
run_benchmark() {
    ...
    if [ "$DATASET_NAME" = "random" ]; then
        DATASET_ARGS="$DATASET_ARGS --random-input-len $RANDOM_INPUT_LEN --random-output-len $RANDOM_OUTPUT_LEN"
        INPUT_LEN_FOR_RESULT=$RANDOM_INPUT_LEN
        OUTPUT_LEN_FOR_RESULT=$RANDOM_OUTPUT_LEN
        EXTRA_BODY='{"ignore_eos": true, "temperature": 0}'
    else
        DATASET_ARGS="$DATASET_ARGS --sharegpt-context-len $SHAREGPT_CONTEXT_LEN"
        if [[ -n "${DATASET_PATH}" ]]; then
            DATASET_ARGS="$DATASET_ARGS --dataset-path ${DATASET_PATH}"
        fi
        INPUT_LEN_FOR_RESULT=0
        OUTPUT_LEN_FOR_RESULT=0
        # Use actual max_tokens from dataset, only set ignore_eos and temperature
        EXTRA_BODY='{"ignore_eos": true, "temperature": 0}'
    fi
...
# Run benchmarks for each deterministic ratio
for ratio in $DETERMINISTIC_RATIOS; do
    echo ""
    echo "========== Deterministic Ratio: $ratio =========="
    ...
    echo "Waiting for all ${#pids[@]} benchmarks to complete..."
    for pid in "${pids[@]}"; do
        wait "$pid"
    done
    echo "All benchmarks for ratio=$ratio completed."
done
...
python "${ROOT}/plot_throughput.py" --results-file "$RESULTS_FILE" --output-dir "$OUTPUT_DIR"
