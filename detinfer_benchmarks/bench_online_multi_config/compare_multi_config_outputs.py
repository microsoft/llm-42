#!/usr/bin/env python3
"""
Compare outputs across multiple configuration types (same QPS, different configs).
Each server runs at the same QPS but with a different determinism configuration,
...
def plot_mismatch_heatmap(mismatch_matrix: np.ndarray, config_names: List[str], output_path: Path):
    ...
    plt.title('Pairwise Output Mismatch Fractions (Different Configs)')
    plt.tight_layout()
...
def main():
    parser = argparse.ArgumentParser(description="Compare outputs across multiple configuration types")
    ...
    parser.add_argument("--dataset-name", default="sharegpt", help="Dataset type: sharegpt, random")
    ...
    parser.add_argument("--random-input-len", type=int, default=1024, help="Input token length for random dataset")
    ...
    #   3. This ensures fair comparison across different configs
    ...
    with mp.Pool(processes=len(base_urls)) as pool:
        # Prepare arguments: (idx, base, url, qps, config_name, output_dir, tokenizer_id)
        args_list = [
    ...
    # Collect per-config rollback stats for summary
    config_rollback_stats = []
    for r in process_results:
        if r.get("rollback_stats"):
            config_rollback_stats.append({
    ...
    for i in range(num_runs):
        for j in range(i + 1, num_runs):
            ...
            summary_mismatches = [
                {
                    ...
                    f"output_len_{config_i}": m[f"output_len_{config_i}"],
                    f"output_len_{config_j}": m[f"output_len_{config_j}"],
                    f"det_num_rollbacks_{config_i}": det_rollbacks_i[m["request_id"]] if det_rollbacks_i and m["request_id"] < len(det_rollbacks_i) else 0,
                    f"det_tokens_rolled_back_{config_i}": det_tokens_rb_i[m["request_id"]] if det_tokens_rb_i and m["request_id"] < len(det_tokens_rb_i) else 0,
                    f"det_num_rollbacks_{config_j}": det_rollbacks_j[m["request_id"]] if det_rollbacks_j and m["request_id"] < len(det_rollbacks_j) else 0,
                    f"det_tokens_rolled_back_{config_j}": det_tokens_rb_j[m["request_id"]] if det_tokens_rb_j and m["request_id"] < len(det_tokens_rb_j) else 0,
            ...
            summary_file = cli_args.output_dir / f"compare_{config_i}_vs_{config_j}_summary.jsonl"
    ...
    summary = {
        ...
        "num_prompts": cli_args.num_prompts,
        "seed": cli_args.seed,
        "config_rollback_stats": config_rollback_stats,
