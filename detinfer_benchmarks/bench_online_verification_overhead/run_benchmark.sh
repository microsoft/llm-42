...
# Verification Overhead Benchmark - Run Single Dataset Config
# Runs the same QPS across servers with different step sizes and skip_mismatch enabled.
#
...
#   CONFIG_NAMES (comma-separated list, default: detinfer_step_size_32,detinfer_step_size_128,detinfer_step_size_512,detinfer_step_size_1024)
#   QPS (default: 12)
...
echo "=============================================="
echo "Verification Overhead Benchmark"
echo "=============================================="
echo "Configuration:"
echo "  Model:           $MODEL"
...
echo ""
echo "NOTE: All servers have skip_mismatch enabled (measuring verification overhead only)"
echo ""
...
echo ""
echo "All servers healthy. Running verification overhead benchmark..."
echo ""

# Use the local compare script
COMPARE_SCRIPT="${ROOT}/compare_verification_overhead.py"

if [ ! -f "$COMPARE_SCRIPT" ]; then
    echo "Error: compare_verification_overhead.py not found at $COMPARE_SCRIPT"
    exit 1
fi
...
if [ $RESULT -eq 0 ]; then
    ...
    if [ -f "$SUMMARY_FILE" ] && command -v jq &> /dev/null; then
        echo "Latency Summary by Step Size:"
