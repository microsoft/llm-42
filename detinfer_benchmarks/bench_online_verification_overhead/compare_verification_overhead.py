#!/usr/bin/env python3
"""
Compare outputs across multiple step sizes for verification overhead measurement.
Each server runs at the same QPS but with a different step size (and skip_mismatch enabled),
and we compare their outputs and latency metrics.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Compare outputs across multiple step sizes for verification overhead")
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("verification_overhead"))
