#!/usr/bin/env python3
"""
Plot latency comparison (TTFT, TPOT, E2E) across different determinism modes and mismatch percentages.
...
def get_short_name(config_name: str) -> str:
    ...
    name_map = {
        "default": "Non-Det",
        "global": "Global-Det",
        "detinfer_512_0pct": "DetInfer-0%",
        "detinfer_512_5pct": "DetInfer-5%",
        "detinfer_512_10pct": "DetInfer-10%",
        "detinfer_512_20pct": "DetInfer-20%",
    }
    return name_map.get(config_name, config_name)
...
def plot_mean_comparison(
    ...
    ax.set_title('Mean Latency Comparison Across Configurations')
    ax.set_xticks(x)
    ax.set_xticklabels(short_names, rotation=15, ha='right')
    ax.legend()
    ax.grid(axis='y', alpha=0.3)
...
def main():
    parser = argparse.ArgumentParser(description="Plot latency comparison for verification percentage")
