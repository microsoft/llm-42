...
def plot_cdf_comparison(raw_data: Dict[str, Dict], output_dir: Path):
    ...
    mode_labels = {
        ...
        'matmul_det_tm': 'Matmul Det (TM)',
        'matmul_det_cutlass': 'Matmul Det (Cutlass)',
        'attention_det_only': 'Attention Det Only',
        'rmsnorm_det_only': 'RMSNorm Det Only',
        'rmsnorm_det_ours': 'RMSNorm Det Ours',
    }
    
    # Colors for different component tests
    colors = {
        'baseline_nondet': '#e74c3c',      # Red - baseline
        'matmul_det_tm': '#3498db',        # Blue - TM matmul
        'matmul_det_cutlass': '#9b59b6',   # Purple - Cutlass matmul
        'attention_det_only': '#2ecc71',   # Green - attention
        'rmsnorm_det_only': '#f39c12',     # Orange - rmsnorm
        'rmsnorm_det_ours': '#e67e22',     # Dark orange - rmsnorm ours
    }
    ...
    line_styles = {
        ...
        'matmul_det_cutlass': '-.',
        'attention_det_only': ':',
        'rmsnorm_det_only': (0, (3, 1, 1, 1)),
        'rmsnorm_det_ours': (0, (5, 2, 1, 2)),
    }
    ...
    mode_order = ['baseline_nondet', 'matmul_det_tm', 'matmul_det_cutlass', 
                  'attention_det_only', 'rmsnorm_det_only', 'rmsnorm_det_ours']
...
def plot_bar_comparison(results: Dict[str, Dict], output_dir: Path):
    ...
    mode_labels = {
        ...
        'matmul_det_tm': 'Matmul Det\n(TM)',
        'matmul_det_cutlass': 'Matmul Det\n(Cutlass)',
        'attention_det_only': 'Attention\nDet Only',
        'rmsnorm_det_only': 'RMSNorm\nDet Only',
        'rmsnorm_det_ours': 'RMSNorm\nDet Ours',
    }
    
    colors = {
        'baseline_nondet': '#e74c3c',
        'matmul_det_tm': '#3498db',
        'matmul_det_cutlass': '#9b59b6',
        'attention_det_only': '#2ecc71',
        'rmsnorm_det_only': '#f39c12',
        'rmsnorm_det_ours': '#e67e22',
    }
...
def plot_overhead_comparison(results: Dict[str, Dict], output_dir: Path):
    ...
    mode_order = ['matmul_det_tm', 'matmul_det_cutlass', 'attention_det_only', 'rmsnorm_det_only', 'rmsnorm_det_ours']
    available_modes = [m for m in mode_order if m in results]
...
def print_summary_table(results: Dict[str, Dict]):
    ...
    if 'baseline_nondet' in results:
        ...
        for mode in ['matmul_det_tm', 'matmul_det_cutlass', 'attention_det_only', 'rmsnorm_det_only', 'rmsnorm_det_ours']:
            if mode not in results:
                continue
...
def main():
    ...
    results = {}
    raw_data = {}
    mode_dirs = ['baseline_nondet', 'matmul_det_tm', 'matmul_det_cutlass', 
                 'attention_det_only', 'rmsnorm_det_only', 'rmsnorm_det_ours']
