...
def plot_indices(records: List[Dict], out_path: Path):
    ...
    plt.plot(xs[:num_records], first_consistent_span[:num_records], label="first_consistent_span", color="tab:blue")
    plt.fill_between(xs[:num_records], 0, first_consistent_span[:num_records], color="tab:blue", alpha=0.06)

    
    plt.plot(xs[:num_records],  second_consistent_span[:num_records], label="second_consistent_span", color="tab:green")
    plt.fill_between(xs[:num_records], 0, second_consistent_span[:num_records], color="tab:green", alpha=0.1)

    plt.plot(xs[:num_records], output_span[:num_records], label="output_span", color="tab:red")
    plt.fill_between(xs[:num_records], 0, output_span[:num_records], color="tab:red", alpha=0.01)
