#!/bin/bash
set -euo pipefail

# Run offline throughput benchmarks against pre-launched servers
# For non-det and global-det: run with det_ratio=1.0
# For detinfer configs: run with multiple det_ratios (0.02, 0.05, 0.1, 0.2, 0.5, 1.0)
#
# Two modes:
#   - SEQUENTIAL_MODE=true (default): Run configs sequentially on a single server (for TP4)
#     Each config will launch its own server, run benchmarks, then shut down
#   - SEQUENTIAL_MODE=false: Run multiple pre-launched servers in parallel

ROOT=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)

# Mode: sequential (one server at a time for TP4) or parallel (multiple servers)
SEQUENTIAL_MODE=${SEQUENTIAL_MODE:-true}

# Server URL (for sequential mode, just one URL)
BASE_URL=${BASE_URL:-"http://127.0.0.1:30005"}
# For parallel mode: comma-separated URLs
BASE_URLS=${BASE_URLS:-"$BASE_URL"}
CONFIG_NAMES=${CONFIG_NAMES:-"sglang_non_deterministic,sglang_global_deterministic,detinfer_ws_64_bs_8"}

# Benchmark parameters
MODEL=${MODEL:-meta-llama/Llama-3.1-8B-Instruct}
TOKENIZER=${TOKENIZER:-}
NUM_PROMPTS=${NUM_PROMPTS:-4096}
DATASET_NAME=${DATASET_NAME:-sharegpt}  # sharegpt, random, or arxiv
DATASET_PATH=${DATASET_PATH:-}
SHAREGPT_CONTEXT_LEN=${SHAREGPT_CONTEXT_LEN:-16384}
RANDOM_INPUT_LEN=${RANDOM_INPUT_LEN:-1024}
RANDOM_OUTPUT_LEN=${RANDOM_OUTPUT_LEN:-128}
DETERMINISTIC_SEED=${DETERMINISTIC_SEED:-42}
BACKEND=${BACKEND:-sglang}

# Deterministic ratios for different config types
BASELINE_RATIOS="1.0"
DETINFER_RATIOS="0.02 0.05 0.1 0.2 0.5 1.0"

# Output directory
TIMESTAMP=$(date +"%Y%m%d_%H%M%S")
if [ "$DATASET_NAME" = "random" ]; then
    OUTPUT_DIR="${ROOT}/results_random_in${RANDOM_INPUT_LEN}_out${RANDOM_OUTPUT_LEN}_n${NUM_PROMPTS}_${TIMESTAMP}"
else
    OUTPUT_DIR="${ROOT}/results_${DATASET_NAME}_n${NUM_PROMPTS}_${TIMESTAMP}"
fi
RESULTS_FILE="${OUTPUT_DIR}/benchmark_results.jsonl"

# Convert comma-separated strings to arrays
IFS=',' read -ra URLS_ARRAY <<< "$BASE_URLS"
IFS=',' read -ra CONFIG_ARRAY <<< "$CONFIG_NAMES"

NUM_CONFIGS=${#CONFIG_ARRAY[@]}

if [ "$SEQUENTIAL_MODE" = true ]; then
    NUM_RUNS=$NUM_CONFIGS
else
    NUM_SERVERS=${#URLS_ARRAY[@]}
    NUM_RUNS=$((NUM_SERVERS < NUM_CONFIGS ? NUM_SERVERS : NUM_CONFIGS))
fi

# Create output directory
mkdir -p "$OUTPUT_DIR"

echo "=============================================="
echo "Offline Throughput Benchmark (v2)"
echo "=============================================="
echo "Mode: $([ "$SEQUENTIAL_MODE" = true ] && echo 'Sequential (single server, TP4)' || echo 'Parallel (multiple servers)')"
echo "Model: $MODEL"
echo "Num Prompts: $NUM_PROMPTS"
echo "Dataset: $DATASET_NAME"
if [ "$DATASET_NAME" = "random" ]; then
    echo "  Input Length: $RANDOM_INPUT_LEN"
    echo "  Output Length: $RANDOM_OUTPUT_LEN"
else
    echo "  Context Length: $SHAREGPT_CONTEXT_LEN"
    echo "  Output Length: (from dataset)"
fi
echo "Baseline Ratios: $BASELINE_RATIOS"
echo "DetInfer Ratios: $DETINFER_RATIOS"
echo "Output Dir: $OUTPUT_DIR"
echo ""
if [ "$SEQUENTIAL_MODE" = true ]; then
    echo "Configs to run sequentially on $BASE_URL:"
    for ((i=0; i<NUM_RUNS; i++)); do
        echo "  $((i+1)). ${CONFIG_ARRAY[$i]}"
    done
else
    echo "Config to Server Mapping:"
    for ((i=0; i<NUM_RUNS; i++)); do
        echo "  ${CONFIG_ARRAY[$i]} -> ${URLS_ARRAY[$i]}"
    done
fi
echo "=============================================="
echo ""

# Function to check server health
check_server_health() {
    local url="$1"
    local max_retries="${2:-60}"  # Default 60 retries (5 minutes with 5s interval)
    local retry_interval="${3:-5}"
    
    for ((retry=1; retry<=max_retries; retry++)); do
        RESPONSE=$(timeout 5 curl -s "${url}/v1/models" 2>&1)
        if echo "$RESPONSE" | grep -q '"object":"list"'; then
            return 0
        fi
        if [ "$retry" -lt "$max_retries" ]; then
            echo "    Waiting for server... (attempt $retry/$max_retries)"
            sleep "$retry_interval"
        fi
    done
    return 1
}

# Skip health check for sequential mode (servers will be launched per-config)
if [ "$SEQUENTIAL_MODE" = false ]; then
    echo "Checking server health..."
    ALL_HEALTHY=true
    for ((i=0; i<NUM_RUNS; i++)); do
        URL="${URLS_ARRAY[$i]}"
        echo -n "  Checking $URL ... "
        
        RESPONSE=$(timeout 5 curl -s "${URL}/v1/models" 2>&1)
        if echo "$RESPONSE" | grep -q '"object":"list"'; then
            echo "✓"
        else
            echo "✗ (not responding)"
            ALL_HEALTHY=false
        fi
    done

    if [ "$ALL_HEALTHY" = false ]; then
        echo ""
        echo "ERROR: Some servers are not healthy. Please launch servers first:"
        echo "  ./launch_servers_parallel.sh"
        exit 1
    fi

    echo ""
    echo "All servers healthy. Running benchmarks..."
    echo ""
fi

# Function to run benchmark for a single server
run_benchmark() {
    local url="$1"
    local config_name="$2"
    local det_ratio="$3"
    
    local temp_result="${OUTPUT_DIR}/temp_${config_name}_det${det_ratio}.jsonl"
    
    echo "[${config_name}] Running benchmark with det_ratio=$det_ratio..."
    
    # Build tokenizer arg if provided
    TOKENIZER_ARG=""
    if [[ -n "${TOKENIZER}" ]]; then
        TOKENIZER_ARG="--tokenizer ${TOKENIZER}"
    fi
    
    # Build dataset args based on dataset type
    if [ "$DATASET_NAME" = "random" ]; then
        DATASET_ARGS="--dataset-name random --random-input-len $RANDOM_INPUT_LEN --random-output-len $RANDOM_OUTPUT_LEN"
        INPUT_LEN_FOR_RESULT=$RANDOM_INPUT_LEN
        OUTPUT_LEN_FOR_RESULT=$RANDOM_OUTPUT_LEN
        EXTRA_BODY='{"ignore_eos": true, "temperature": 0}'
    else
        # sharegpt, arxiv, and other datasets
        DATASET_ARGS="--dataset-name ${DATASET_NAME} --sharegpt-context-len $SHAREGPT_CONTEXT_LEN"
        if [[ -n "${DATASET_PATH}" ]]; then
            DATASET_ARGS="$DATASET_ARGS --dataset-path ${DATASET_PATH}"
        fi
        INPUT_LEN_FOR_RESULT=0
        OUTPUT_LEN_FOR_RESULT=0
        EXTRA_BODY='{"ignore_eos": true, "temperature": 0}'
    fi
    
    python -m sglang.bench_serving \
        --backend "$BACKEND" \
        --base-url "$url" \
        --model "$MODEL" \
        $TOKENIZER_ARG \
        $DATASET_ARGS \
        --num-prompts "$NUM_PROMPTS" \
        --request-rate inf \
        --deterministic-ratio "$det_ratio" \
        --deterministic-seed "$DETERMINISTIC_SEED" \
        --extra-request-body "$EXTRA_BODY" \
        --output-file "$temp_result" \
        --output-details \
        2>&1 | tee "${OUTPUT_DIR}/log_${config_name}_det${det_ratio}.log"
    
    # Extract metrics and append to results
    if [ -f "$temp_result" ]; then
        # Parse the JSONL output and add metadata including rollback stats
        python -c "
import json
import sys

with open('$temp_result', 'r') as f:
    for line in f:
        line = line.strip()
        if line:
            try:
                result = json.loads(line)
                result['config_name'] = '$config_name'
                result['dataset_name'] = '$DATASET_NAME'
                result['input_len'] = $INPUT_LEN_FOR_RESULT
                result['output_len'] = $OUTPUT_LEN_FOR_RESULT
                result['deterministic_ratio'] = $det_ratio
                result['server_url'] = '$url'
                
                # Extract rollback stats from meta_info
                meta_info_list = result.get('meta_info', [])
                output_lens = result.get('output_lens', [])
                if meta_info_list:
                    det_num_rollbacks = [m.get('det_infer_num_rollbacks', 0) for m in meta_info_list if m]
                    det_tokens_rolled_back = [m.get('det_infer_tokens_rolled_back', 0) for m in meta_info_list if m]
                    
                    num_requests = len(det_num_rollbacks)
                    total_output_tokens = sum(output_lens) if output_lens else result.get('total_output_tokens', 0)
                    if num_requests > 0:
                        result['rollback_stats'] = {
                            'total_rollbacks': sum(det_num_rollbacks),
                            'total_tokens_rolled_back': sum(det_tokens_rolled_back),
                            'total_output_tokens': total_output_tokens,
                            'avg_rollbacks_per_request': sum(det_num_rollbacks) / num_requests,
                            'avg_tokens_rolled_back_per_request': sum(det_tokens_rolled_back) / num_requests,
                            'max_rollbacks_per_request': max(det_num_rollbacks) if det_num_rollbacks else 0,
                            'max_tokens_rolled_back_per_request': max(det_tokens_rolled_back) if det_tokens_rolled_back else 0,
                            'requests_with_rollbacks': sum(1 for x in det_num_rollbacks if x > 0),
                            'num_requests': num_requests,
                        }
                
                # Remove verbose fields to keep results file manageable
                for key in ['meta_info', 'generated_texts', 'output_ids', 'itls', 'errors']:
                    result.pop(key, None)
                
                print(json.dumps(result))
            except json.JSONDecodeError:
                pass
" >> "$RESULTS_FILE"
        rm -f "$temp_result"
    fi
    
    echo "[${config_name}] Completed det_ratio=$det_ratio"
}

# Function to run all benchmarks for a single server
run_server_benchmarks() {
    local url="$1"
    local config_name="$2"
    
    if [[ "$config_name" == *"detinfer"* ]]; then
        # DetInfer: run all ratios sequentially
        for ratio in $DETINFER_RATIOS; do
            run_benchmark "$url" "$config_name" "$ratio"
        done
    else
        # Baseline: run only with ratio 1.0
        run_benchmark "$url" "$config_name" "1.0"
    fi
}

# Function to launch a server for a specific config (sequential mode only)
launch_server_for_config() {
    local config_name="$1"
    local port="${2:-30005}"
    
    # Kill any existing process on this port
    local existing_pid
    existing_pid=$(lsof -ti:$port 2>/dev/null || true)
    if [ -n "$existing_pid" ]; then
        echo "  Killing existing process on port $port (PID: $existing_pid)..."
        kill $existing_pid 2>/dev/null || true
        sleep 2
    fi
    
    # Get config-specific args
    local config_args
    case "$config_name" in
        "sglang_non_deterministic")
            config_args=""
            ;;
        "sglang_global_deterministic")
            config_args="--enable-deterministic-inference 2"
            ;;
        "detinfer_ws_32_bs_16")
            config_args="--det-infer-window-size 32 --enable-det-infer 3 --det-infer-verify-batch-size 16"
            ;;
        "detinfer_ws_16_bs_32")
            config_args="--det-infer-window-size 16 --enable-det-infer 3 --det-infer-verify-batch-size 32"
            ;;
        "detinfer_ws_64_bs_16")
            config_args="--det-infer-window-size 64 --enable-det-infer 3 --det-infer-verify-batch-size 16"
            ;;
        "detinfer_ws_64_bs_8")
            config_args="--det-infer-window-size 64 --enable-det-infer 3 --det-infer-verify-batch-size 8"
            ;;
        "detinfer_ws_32_bs_32")
            config_args="--det-infer-window-size 32 --enable-det-infer 3 --det-infer-verify-batch-size 32"
            ;;
        *)
            echo "Error: Unknown config name: $config_name" >&2
            exit 1
            ;;
    esac
    
    local log_file="${OUTPUT_DIR}/server_${config_name}.log"
    
    echo "  Launching server with config: $config_name"
    echo "  Config args: $config_args"
    echo "  Log file: $log_file"
    
    # Server parameters (can be overridden via env vars)
    local model_path="${SGLANG_TEST_MODEL:-$MODEL}"
    local host="${SGLANG_HOST:-0.0.0.0}"
    local tp_size="${SGLANG_TP_SIZE:-4}"
    local attention_backend="${SGLANG_ATTENTION_BACKEND:-fa3}"
    
    python -m sglang.launch_server \
        --model-path "$model_path" \
        --host "$host" \
        --port "$port" \
        --tp "$tp_size" \
        --attention-backend "$attention_backend" \
        --disable-radix-cache \
        --disable-chunked-prefix-cache \
        --disable-overlap-schedule \
        --enable-metrics \
        --random-seed 42 \
        --chunked-prefill-size -1 \
        $config_args \
        > "$log_file" 2>&1 &
    
    SERVER_PID=$!
    echo "  Server PID: $SERVER_PID"
    
    # Wait for server to be ready
    echo "  Waiting for server to be ready..."
    if ! check_server_health "$BASE_URL" 120 5; then
        echo "  ERROR: Server failed to start. Check log: $log_file"
        kill $SERVER_PID 2>/dev/null || true
        exit 1
    fi
    echo "  Server ready!"
}

# Function to stop the server (sequential mode only)
stop_server() {
    if [ -n "${SERVER_PID:-}" ]; then
        echo "  Stopping server (PID: $SERVER_PID)..."
        # Kill the main process and all children (important for TP>1)
        pkill -P $SERVER_PID 2>/dev/null || true
        kill $SERVER_PID 2>/dev/null || true
        wait $SERVER_PID 2>/dev/null || true
        sleep 5  # Give time for GPU memory cleanup
        echo "  Server stopped."
        SERVER_PID=""
    fi
}

# Cleanup function for sequential mode
cleanup_server() {
    if [ "$SEQUENTIAL_MODE" = true ] && [ -n "${SERVER_PID:-}" ]; then
        echo ""
        echo "Cleaning up server..."
        pkill -P $SERVER_PID 2>/dev/null || true
        kill $SERVER_PID 2>/dev/null || true
    fi
}
trap cleanup_server EXIT INT TERM

if [ "$SEQUENTIAL_MODE" = true ]; then
    # Sequential mode: launch server, run benchmarks, stop server, repeat
    echo "========== Running Configs Sequentially (TP4 Mode) =========="
    echo "Baseline configs: det_ratio=1.0"
    echo "DetInfer configs: det_ratios=$DETINFER_RATIOS"
    echo ""
    
    # Extract port from BASE_URL
    PORT=$(echo "$BASE_URL" | sed -E 's/.*:([0-9]+)$/\1/')
    
    for ((i=0; i<NUM_RUNS; i++)); do
        config_name="${CONFIG_ARRAY[$i]}"
        
        echo ""
        echo "==================== Config $((i+1))/$NUM_RUNS: $config_name ===================="
        
        # Launch server for this config
        launch_server_for_config "$config_name" "$PORT"
        
        # Run benchmarks
        echo "  Running benchmarks..."
        run_server_benchmarks "$BASE_URL" "$config_name"
        
        # Stop server
        stop_server
        
        echo "==================== Completed: $config_name ===================="
    done
else
    # Parallel mode: run against pre-launched servers
    echo "========== Running All Servers in Parallel =========="
    echo "Baseline configs: det_ratio=1.0"
    echo "DetInfer configs: det_ratios=$DETINFER_RATIOS"
    echo ""

    pids=()
    for ((i=0; i<NUM_RUNS; i++)); do
        url="${URLS_ARRAY[$i]}"
        config_name="${CONFIG_ARRAY[$i]}"
        
        echo "Starting benchmarks for $config_name on $url..."
        run_server_benchmarks "$url" "$config_name" &
        pids+=($!)
    done

    # Wait for all servers to complete
    echo ""
    echo "Waiting for all servers to complete..."
    for pid in "${pids[@]}"; do
        wait "$pid"
    done
fi

echo ""
echo "=============================================="
echo "Benchmarking Complete!"
echo "Results saved to: $RESULTS_FILE"
echo "=============================================="
# Export per-request data to CSV
CSV_FILE="${OUTPUT_DIR}/per_request_data.csv"
echo ""
echo "Exporting per-request data to CSV..."
python "${ROOT}/export_per_request_csv.py" \
    --input "$RESULTS_FILE" \
    --output "$CSV_FILE"

echo ""
echo "Per-request CSV: $CSV_FILE"
echo "Fields: config_name, dataset_name, det_ratio, request_idx, prompt_hash,"
echo "        is_deterministic, input_len, output_len, rollbacks, tokens_rolled_back,"
echo "        ttft_s, latency_s, error"