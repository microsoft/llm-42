...
# Usage: NUM_GPUS=1 TP_SIZE=4 GPU_LIST=0,1,2,3 ./run_online_benchmarks.sh --start-servers
...
# Configuration
# ============================================
MODEL="${SGLANG_TEST_MODEL:-Qwen/Qwen3-30B-A3B}"
...
OUTPUT_DIR="$(dirname "$0")/Qwen3-30B-A3B"
...
# Enforce single-server batches when TP > 1 to avoid GPU oversubscription
if [ "$TP_SIZE" -gt 1 ] && [ "$NUM_GPUS" -ne 1 ]; then
    echo "TP_SIZE=$TP_SIZE requires NUM_GPUS=1; overriding NUM_GPUS to 1."
    NUM_GPUS=1
fi
...
launch_server() {
    ...
    echo "Launching $name on GPUs $GPU_LIST, port $port (tp=$TP_SIZE)..."
    CUDA_VISIBLE_DEVICES=$GPU_LIST $PYTHON_CMD -m sglang.launch_server \
        --model-path "$MODEL" --host "$HOST" --port "$port" --tp "$TP_SIZE" \
...
[[ "$1" != "--start-servers" ]] && { echo "Usage: NUM_GPUS=1 TP_SIZE=4 GPU_LIST=0,1,2,3 $0 --start-servers"; exit 1; }
