...
def build_base_args(cli_args: argparse.Namespace) -> dict:
    base_dict = {
        ...
        "deterministic_seed": cli_args.seed,
        ...
        "select_seed": cli_args.seed,
        "order_seed": cli_args.seed,
        "arrival_seed": cli_args.seed,
...
def run_experiment_process(idx: int, base: dict, url: str, qps: float, step_size: int,
                          output_dir: Path, tokenizer_id: str) -> dict:
    """Run a single experiment in a separate process."""
    import copy
    import sys
    
    print(f"[{idx+1}] Starting step_size={step_size} (QPS={qps}) on {url}...", flush=True)
    output_file = output_dir / f"step_size_{step_size}.jsonl"
    ...
    print(f"[{idx+1}] step_size={step_size} benchmark completed, tokenizing outputs...", flush=True)
    
    # Tokenize outputs
    tokenizer = bench_serving.get_tokenizer(tokenizer_id or args.model)
    print(f"[{idx+1}] step_size={step_size} tokenizer loaded", flush=True)
    tokens = tokenize(result["generated_texts"], tokenizer)
    print(f"[{idx+1}] step_size={step_size} tokenization done", flush=True)
    ...
    print(f"[{idx+1}] step_size={step_size} completed: {len(tokens)} responses")
    if rollback_stats:
        print(f"[{idx+1}] step_size={step_size} rollback stats: {rollback_stats['total_rollbacks']} rollbacks, "
...
def plot_mismatch_heatmap(mismatch_matrix: np.ndarray, step_sizes: List[int], output_path: Path):
    """Plot heatmap showing fraction of mismatches between each pair of step size values."""
    plt.figure(figsize=(8, 6))
    im = plt.imshow(mismatch_matrix, cmap='RdYlGn_r', aspect='auto', vmin=0, vmax=1)
    plt.colorbar(im, label='Mismatch Fraction')
    
    labels = [f"step={s}" for s in step_sizes]
    plt.xticks(range(len(step_sizes)), labels, rotation=45, ha='right')
    plt.yticks(range(len(step_sizes)), labels)
    ...
    plt.title('Pairwise Output Mismatch Fractions (Different Step Sizes)')
    plt.tight_layout()
...
def main():
    parser = argparse.ArgumentParser(description="Compare outputs across multiple step size configurations")
    ...
    parser.add_argument("--step-sizes", required=True, help="Comma-separated list of step sizes")
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("multi_step_size_comparison"))
    ...
    # Parse URLs and step sizes
    base_urls = cli_args.base_urls.split(',')
    step_sizes = [int(x) for x in cli_args.step_sizes.split(',')]
    qps = cli_args.qps
    
    if len(base_urls) != len(step_sizes):
        print(f"Error: Number of URLs ({len(base_urls)}) must match number of step sizes ({len(step_sizes)})")
        return 1
    
    print(f"Running {len(base_urls)} experiments at QPS={qps}:")
    for url, step_size in zip(base_urls, step_sizes):
        print(f"  step_size={step_size} on {url}")
    print()
    ...
    #   3. This ensures fair comparison across different step sizes
    ...
    with mp.Pool(processes=len(base_urls)) as pool:
        # Prepare arguments: (idx, base, url, qps, step_size, output_dir, tokenizer_id)
        args_list = [
            (i, base, url, qps, step_size, cli_args.output_dir, tokenizer_id)
            for i, (url, step_size) in enumerate(zip(base_urls, step_sizes))
        ]
    ...
    # Collect per-step-size rollback stats for summary
    step_size_rollback_stats = []
    for r in process_results:
        if r.get("rollback_stats"):
            step_size_rollback_stats.append({
    ...
    tokenized_outputs = [
        (r["step_size"], r["tokens"], r["texts"], r["prompt_lens"], 
         r.get("det_num_rollbacks", []), r.get("det_tokens_rolled_back", []))
        for r in process_results
    ]
    
    # Compare all pairs
    num_runs = len(step_sizes)
    ...
    for i in range(num_runs):
        for j in range(i + 1, num_runs):
            step_i, tokens_i, texts_i, prompt_lens_i, det_rollbacks_i, det_tokens_rb_i = tokenized_outputs[i]
            step_j, tokens_j, texts_j, prompt_lens_j, det_rollbacks_j, det_tokens_rb_j = tokenized_outputs[j]
            ...
            for req_id, (tok_i, tok_j, text_i, text_j, plen_i, plen_j) in enumerate(zip(tokens_i, tokens_j, texts_i, texts_j, prompt_lens_i, prompt_lens_j)):
                ...
                mismatches.append({
                    "request_id": req_id,
                    "prompt_len": plen_i,  # Should be same for both with same seed
                    f"output_len_step_{step_i}": output_len_i,
                    f"output_len_step_{step_j}": output_len_j,
                    f"step_{step_i}_tokens": tok_i,
                    f"step_{step_j}_tokens": tok_j,
                    f"step_{step_i}_text": text_i,
                    f"step_{step_j}_text": text_j,
            ...
            pair_file = cli_args.output_dir / f"compare_step_{step_i}_vs_{step_j}.jsonl"
            ...
            summary_mismatches = [
                {
                    ...
                    f"output_len_step_{step_i}": m[f"output_len_step_{step_i}"],
                    f"output_len_step_{step_j}": m[f"output_len_step_{step_j}"],
                    f"det_num_rollbacks_step_{step_i}": det_rollbacks_i[m["request_id"]] if det_rollbacks_i and m["request_id"] < len(det_rollbacks_i) else 0,
                    f"det_tokens_rolled_back_step_{step_i}": det_tokens_rb_i[m["request_id"]] if det_tokens_rb_i and m["request_id"] < len(det_tokens_rb_i) else 0,
                    f"det_num_rollbacks_step_{step_j}": det_rollbacks_j[m["request_id"]] if det_rollbacks_j and m["request_id"] < len(det_rollbacks_j) else 0,
                    f"det_tokens_rolled_back_step_{step_j}": det_tokens_rb_j[m["request_id"]] if det_tokens_rb_j and m["request_id"] < len(det_tokens_rb_j) else 0,
            ...
            summary_file = cli_args.output_dir / f"compare_step_{step_i}_vs_{step_j}_summary.jsonl"
            ...
            print(f"step_size {step_i} vs step_size {step_j}: {mismatch_frac:.2%} mismatch rate, "
                  f"{num_delta_gt_64} deltas > 64, {num_delta_gt_128} deltas > 128")
    
    # Plot heatmap
    heatmap_path = cli_args.output_dir / "mismatch_heatmap.pdf"
    plot_mismatch_heatmap(mismatch_matrix, step_sizes, heatmap_path)
    ...
    summary = {
        ...
        "num_prompts": cli_args.num_prompts,
        "seed": cli_args.seed,
        "step_size_rollback_stats": step_size_rollback_stats,
